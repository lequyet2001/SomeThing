...
import type { CartItem, EntityId, Product, SetNoticeFn, User } from '../../../types/shop'
...
function animateProductToCart(sourceElement: HTMLElement | null) {
...
export function useCartActions({ dispatch, setNotice, user }: CartActionsContext) {
  ...
  async function addToCart(productId: EntityId, quantity = 1, sourceElement: HTMLElement | null = null) {
  ...
  async function updateQuantity(productId: EntityId, quantity: number) {
  ...
  async function removeFromCart(productId: EntityId) {
