...
function createSportsCar(materials) {
...
function createObstacleModel(materials) {
...
function HiddenGamePage() {
  ...
  const [difficultyKey, setDifficultyKey] = useState(DEFAULT_DIFFICULTY.key)
  ...
  useEffect(() => {
    ...
    renderer.domElement.className = 'hidden-game-canvas'
    ...
    const wheelMeshes = player.userData.animatedWheels || []
    ...
    function removeActiveObject(entry) {
  ...
  return (
    <section className="hidden-game-page" aria-label="Atelier Drift">
      <div className="hidden-game-stage" ref={mountRef} />

      <div className="hidden-game-hud">
        <div>
          <span>Score</span>
        ...
        <div className={hud.shield > 0 ? 'is-active' : ''}>
          <span>Shield</span>
          <strong>{hud.shield}</strong>
        </div>
      </div>

      <div className="hidden-game-title">
        <p>Atelier Drift</p>
      </div>

      {!isRunning && (
        <div className="hidden-game-difficulty" aria-label="Difficulty">
          {DIFFICULTY_OPTIONS.map((option) => (
            <button
              type="button"
              className={difficultyKey === option.key ? 'is-active' : ''}
              key={option.key}
      ...
      {(!isRunning || isGameOver) && (
        <button type="button" className="hidden-game-start" onClick={handleStart}>
          {isGameOver ? <RotateCcw size={18} /> : <Play size={18} />}
          {isGameOver ? 'Restart' : 'Start'}
        </button>
      )}

      <div className="hidden-game-controls" aria-label="Game controls">
