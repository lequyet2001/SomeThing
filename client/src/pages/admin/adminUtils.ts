...
export const orderCustomerTypeOptions = [
  { value: 'registered', labelKey: 'admin.customerType.registered' },
  { value: 'guest', labelKey: 'admin.customerType.guest' },
]
...
export function formatAdminDate(value: string | undefined, language: LanguageCode, emptyText: string) {
...
export function isWithinNumberRange(value: number | string | undefined, minValue: string, maxValue: string) {
...
export function getDateBoundary(value: string, time: string) {
  if (!value) return null
  const timestamp = new Date(`${value}T${time}`).getTime()
  return Number.isNaN(timestamp) ? null : timestamp
}

export function isWithinDateRange(value: string | undefined, startDate: string, endDate: string) {
...
export function countStatuses(statusStats: StatusStat[] = [], statuses: string[]) {
  return statusStats
    .filter((item) => statuses.includes(item.status))
    .reduce((sum, item) => sum + (Number(item.count) || 0), 0)
}

export function buildSummaryParams(filters: StatsFilters) {
...
import type { AdminFilters, LanguageCode, StatsFilters, StatusStat } from '../../types/shop'
